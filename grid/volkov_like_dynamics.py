...
r_min = 0
r_max = 80
N = int(3.0 * r_max)  # Number of grid points, should be large enough
...
np.savez(
    f"CN_volkov_dt={dt}_rmax={r_max}_N={N}_hydrogenic_E0={E0}_omega={omega}_ncycles={n_cycles}",
    **samples,
)
...
axs[1, 0].axvline(0.8 * r_max, color="k", ls="--", label=r"$r_{mask}$")
axs[1, 0].grid()
axs[1, 0].legend()
